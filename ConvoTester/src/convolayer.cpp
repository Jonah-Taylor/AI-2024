...
MatrixXd ConvoLayer::forward(const MatrixXd& in) {
    ...
    z = convolve(in_mapped, w) + b;
    a = a_func(z);

    MatrixXd a_mapped = Map<Matrix<double, 728, 1>>(a.data());
...
void ConvoLayer::getOutputDeltas(const MatrixXd& target) {
    ...
    MatrixXd t = Map<const Matrix<double, 28, 28>>(target.data());
...
void ConvoLayer::updateGrads(const MatrixXd& in) {
    ...
    MatrixXd in_mapped = Map<const Matrix<double, 28, 28>>(in.data());
    MatrixXd dz_flipped = dz.colwise().reverse().rowwise().reverse();
    avg_grad_w = MatrixXd::Zero(k_size, k_size);

    // Perform the convolution to calculate the gradients
    for (int i = 0; i <= in_mapped.rows() - k_size; ++i) {
        for (int j = 0; j <= in_mapped.cols() - k_size; ++j) {
            avg_grad_w += (in_mapped.block(i, j, k_size, k_size).array() * dz_flipped.block(i, j, k_size, k_size).array()).matrix();
        }
    }
    avg_grad_b = dz_flipped;
