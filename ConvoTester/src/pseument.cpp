...
NeuralNetwork::NeuralNetwork(const vector<MakeLayer>& l_info) {
    ...
    for (size_t l = 1; l < l_info.size(); ++l) {
        switch(l_info[l].l_type) {
            case 0:
                layers.push_back(unique_ptr<Layer>(new DenseLayer(l_info[l].l_size[0], l_info[l - 1].l_size[0], l_info[l].a_func_name)));
                break;
            case 1:
                layers.push_back(unique_ptr<ConvoLayer>(new ConvoLayer(l_info[l].l_size[0], l_info[l].l_size[1], l_info[l].a_func_name, 5)));
                break;
            default:
                layers.push_back(unique_ptr<Layer>(new DenseLayer(l_info[l].l_size[0], l_info[l - 1].l_size[0], l_info[l].a_func_name)));
                break;
...
MatrixXd NeuralNetwork::forward(const MatrixXd& in) {
...
void NeuralNetwork::getOutputDeltas(const MatrixXd& target) {
