...
void NeuralNetwork::initializeAdamStates() {
    ...
    for (size_t l = 1; l < layers.size(); ++l) {
        for (int i = 0; i < layers[l]; ++i) {
            for (int j = 0; j < layers[l - 1]; ++j) {
                ...
                double grad_w = deltas[l][i] * activations[l - 1][j];
            ...
            biases[l - 1][i] -= learning_rate * m_hat_b / (sqrt(v_hat_b) + epsilon);
        }
    }
}
