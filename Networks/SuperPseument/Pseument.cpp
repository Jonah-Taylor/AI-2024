...
VectorXd NeuralNetwork::forward(const VectorXd& input) {
    ...
    for (size_t l = 1; l < layers.size(); ++l) {
        ...
        activations[l] = sigmoid(z_vals[l]);
    }
...
void NeuralNetwork::getOutputDeltas(vector<VectorXd>& deltas, const VectorXd& target) {
    ...
    deltas.back() = error.array().cwiseProduct(sigmoidDerivative(z_vals.back()).array()); // Element-wise product
...
void NeuralNetwork::backward(vector<VectorXd>& deltas, vector<MatrixXd>& avg_grad_w, 
    ...
    for (uint l = layers.size() - 1; l > 0; --l) {
        ...
        if(l != layers.size() - 1) {
            ...
            deltas[l] = (weights[l + 1].matrix().transpose() * deltas[l + 1].matrix()).array() * sigmoidDerivative(z_vals[l]).array();
...
void NeuralNetwork::updateNeurons(std::vector<MatrixXd>& avg_grad_w, 
    ...
    for (size_t l = 1; l < layers.size(); ++l) {
        ...
        MatrixXd grad_w = avg_grad_w[l];  // Average gradient for weights
...
void NeuralNetwork::train(vector<vector<double>>& X, vector<vector<double>>& Y, 
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        if (print) {
            std::cout << "Epoch " << epoch + 1 << ": " << correct << " / " << tested << "\n";
        }
