...
// Description: Pseument (Pseudo Mantis) is my first AI framework
...
vector<double> NeuralNetwork::forward(const vector<double>& input) {
    ...
    return vector<double>(activations.back().data(), activations.back().data() + activations.back().size());
}
...
void NeuralNetwork::train(vector<vector<double>>& X, vector<vector<double>>& Y, 
    ...
    lambda = learning_rate / 100; // Weight decay to prevent overfitting
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (uint i = 0; i < inputs.rows(); i += batch_size) {
            ...
            for (int b = 0; b < std::min(batch_size, static_cast<int>(inputs.rows() - i)); ++b) {
            ...
            stepSGD(avg_grad_w, avg_grad_b, learning_rate, batch_size); 
        }
