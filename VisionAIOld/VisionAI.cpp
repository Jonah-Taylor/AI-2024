        ...
        if (elapsed >= targetTime || fast) {
            ...
            while (window.pollEvent(event)) {
                if (event.type == sf::Event::Closed) {
                    ...
                    nn.save("Saved/exit" + to_string(id) + "_" + to_string(int(epochs)) + ".txt");
                    window.close();
                }
            }
            ...
            if(training) {
                ...
                nn.train(X, Y, epochs, batchSize, trainingSpeed, printEpochs);
                X.clear();
                Y.clear();
            }
            else {
                ...
                if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
                    for (int dy = -draw_radius; dy <= draw_radius; ++dy) {
                        for (int dx = -draw_radius; dx <= draw_radius; ++dx) {
                            ...
                            if (dx * dx + dy * dy < draw_radius * draw_radius && gridX >= dx && gridX < largeGridSize - dx && gridY >= dy && gridY < largeGridSize - dy) {
...
void keyBoardInputs() {
    ...
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) {
        if(!S) {
            ...
            nn.save("Saved/nw" + to_string(id) + "_" + to_string(int(epochs)) + ".txt");
        }
        S = true;
    }
    else {
